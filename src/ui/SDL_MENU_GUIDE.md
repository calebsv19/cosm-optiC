...
- 3D Render Scale (HiDPI, 1–8, only in 3D mode): native `3D` internal render scale. `HiDPI` is the automatic default: it traces at the window drawable pixel size when the display exposes more pixels than the logical window and otherwise uses the logical size. `1x` always traces at the logical host resolution; higher values trace fewer pixels and reconstruct back into the normal window through the active `3D Upscale Mode`.
