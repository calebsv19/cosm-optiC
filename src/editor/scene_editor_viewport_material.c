...
/* Fixed storage: ~14 MiB, no allocations or texture-stack evaluation during orbit. */
typedef struct Sample { float r,g,b,rough,reflection,specular,diffuse; } Sample;
...
const SceneEditorViewportMaterial* SceneEditorViewportMaterialPrepare(int index) {
    ...
    for(int i=0;i<CACHE_SLOTS;++i) {
        if(cache[i].valid && cache[i].object_index==index && cache[i].hash==hash) {cache[i].stamp=++clock_tick;return &cache[i];}
        if(cache[i].stamp<slot->stamp) slot=&cache[i];
    }
    slot->valid=true;slot->hash=hash;slot->object_index=index;slot->stamp=++clock_tick;
    slot->emission=clamp01(object->emissiveStrength);
    for(int y=0;y<GRID;++y) for(int x=0;x<GRID;++x) {
        RuntimeMaterialSurfaceEval eval=base;
        RuntimeMaterialTextureStackEvaluatePlacedUV(&stack,object,(double)x/(GRID-1),(double)y/(GRID-1),seed,&base,&eval);
        slot->samples[y*GRID+x]=(Sample){eval.colorR,eval.colorG,eval.colorB,eval.roughness,eval.reflectivity,eval.specWeight,eval.diffuseWeight};
    }
    ++builds;return slot;
}
...
#define MIX(field) s.field=(a.field*(1-fx)+b.field*fx)*(1-fy)+(c.field*(1-fx)+d.field*fx)*fy
    MIX(r);MIX(g);MIX(b);MIX(rough);MIX(reflection);MIX(specular);MIX(diffuse);
#undef MIX
