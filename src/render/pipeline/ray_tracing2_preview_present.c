...
static bool BeginNative3DEnvPresentationCapture(void) {
...
static void WriteNative3DEnvPresentationCapture(bool enabled,
                                                bool render_ok,
...
static void RecordNative3DPresentationEvent(
    RayTracing2Native3DPresentationEventKind kind,
...
bool RayTracing2PreviewPresent_ReconstructNative3DHostTruth(
    ...
    if (upscale_mode == RUNTIME_3D_UPSCALE_MODE_OFF) {
...
static bool PresentNative3DPreviewTileProgress(
    ...
    if (!progress || !ctx || !ctx->hostBuffer || !ctx->renderBuffer) {
        return false;
    }
    ...
    if (!PresentNative3DTilePreviewFrameTimed(ctx->renderer,
                                              &ctx->previewCtx,
                                              present_tile,
                                              ctx->hostBuffer,
                                              false)) {
    ...
    if (ctx->stats) {
        ctx->stats->temporalDirtyPreviewPresentCount += 1;
    }
    return true;
}
...
bool RayTracing2PreviewPresent_RenderNative3DTilesPreview(
    ...
    RuntimeNative3DFillPixelBufferBackground(render_buffer,
                                             render_width,
                                             render_height,
                                             &frame.scene,
                                             &frame.projector);
