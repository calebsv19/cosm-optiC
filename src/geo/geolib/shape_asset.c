...
bool shape_asset_rasterize(const ShapeAsset *asset,
    ...
    if (opts) {
        ...
        pos_x = opts->position_x_norm;
        pos_y = opts->position_y_norm;
    ...
    float scale = user_scale;
    float ox = pos_x * (float)(grid_w - 1);
    float oy = pos_y * (float)(grid_h - 1);
    float cx = 0.5f * (bounds.min_x + bounds.max_x);
    float cy = 0.5f * (bounds.min_y + bounds.max_y);
    if (center_fit) {
        fit_transform(&bounds, grid_w, grid_h, margin, &scale, &ox, &oy, &cx, &cy);
    }
    ...
    for (size_t pi = 0; pi < asset->path_count; ++pi) {
        const ShapeAssetPath *p = &asset->paths[pi];
        for (size_t vi = 1; vi < p->point_count; ++vi) {
            rasterize_segment(mask_out, grid_w, grid_h,
                              p->points[vi - 1], p->points[vi],
                              cx, cy, cos_a, sin_a,
                              scale, ox, oy, stroke);
        }
        fill_polygon(mask_out, grid_w, grid_h, p, cx, cy, cos_a, sin_a, scale, ox, oy);
    }
