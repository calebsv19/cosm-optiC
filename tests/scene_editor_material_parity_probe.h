...
static void material_parity_probe(SceneEditor* editor, bool edit) {
    ...
    for(int face=0;face<6;++face) {
        ...
        fprintf(va,"P6\n256 256\n255\n");fprintf(ra,"P6\n256 256\n255\n");
        double error=0;int count=0,different=0,group=-1;
        for(int y=0;y<256;++y) for(int x=0;x<256;++x) {
            ...
            if(RuntimeRay3D_TraceSceneFirstHit(&scene,&ray,.001,20,&hit) && hit.sceneObjectIndex==0) {
                ...
                assert(MaterialPreviewSurfaceEvaluateObject(&sceneSettings.sceneObjects[0],0,NULL,u,v,&eval));
        ...
        fclose(va);fclose(ra);
        fprintf(report,"%s{\"probe_face\":%d,\"runtime_face_group\":%d,\"hits\":%d,\"mae_255\":%.6f,\"different_fraction\":%.6f}",face?",":"",face,group,count,count?error/(count*3):0,count?(double)different/count:0);
    }
