...
int main(void) {
    ...
    for(size_t i=0;i<2;i++) { ProceduralSurfaceFeatureRootV1 *r=&f.features[i]; r->source_triangle=(uint32_t)i; r->barycentric[0]=1; r->normal.z=1; r->tangent.x=1; r->bitangent.y=1; r->radius=.5; r->aspect=1; r->edge_softness=.15; r->rim_width=.2; r->feature_id=(uint32_t)(100+i); }
    ...
    assert(a.feature_id==100 && a.coverage>0 && a.rim>0 && a.interior>0 && a.rim!=a.interior && a.candidates_considered==1);
