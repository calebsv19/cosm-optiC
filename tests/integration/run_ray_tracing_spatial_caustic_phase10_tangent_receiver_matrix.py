#!/usr/bin/env python3
"""Run the Phase 10 tangent-receiver surface-caustic matrix."""
...
def default_review_root() -> Path:
    return workspace_root() / "_private_workspace_artifacts" / "agent_runs" / "ray_tracing" / "caustic_phase10_tangent_receiver_matrix"
...
def matrix_requests(review_root: Path) -> list[dict]:
    ...
    scene_path = repo_root() / "tests" / "fixtures" / "caustic_probe_glass_sphere" / "scene_runtime.json"
    specs = [
        {
            "cell_id": "off_tangent_receiver",
            "run_id": "caustic_phase10_off_tangent_receiver",
        ...
        {
            "cell_id": "surface_cache_tangent_receiver_no_fallback",
            "run_id": "caustic_phase10_surface_cache_tangent_receiver_no_fallback",
            "scene_path": scene_path,
            "inspection": {
...
def validate_cell(cell_id: str, digest: dict) -> list[str]:
    ...
    if cell_id.startswith("off_"):
        ...
        if digest["surface_receiver_fallback_enabled"]:
            failures.append("surface-cache cell unexpectedly enabled receiver fallback")
...
def write_index(path: Path, report: dict) -> None:
    ...
    lines = [
        "# Phase 10 Tangent Receiver Matrix",
        "",
...
def main() -> int:
    ...
    for cell_id in ("surface_cache_tangent_receiver_no_fallback",):
        ...
        if runs_by_cell.get(cell_id) and frame_delta.get("max_luma_delta", 0.0) <= 0.0:
            failures.append(f"{cell_id}: max frame luma delta did not rise above paired off baseline")
    report = {
        "schema_version": "ray_tracing_spatial_caustic_phase10_tangent_receiver_matrix_v1",
    ...
    write_json(review_root / "phase10_tangent_receiver_matrix_report.json", report)
    write_index(review_root / "phase10_tangent_receiver_matrix_index.md", report)
    print(review_root / "phase10_tangent_receiver_matrix_report.json")
