#!/usr/bin/env python3
"""Run the Phase 7 surface-caustic calibration matrix."""
...
def default_review_root() -> Path:
    return workspace_root() / "_private_workspace_artifacts" / "agent_runs" / "ray_tracing" / "caustic_phase7_calibration_matrix"
...
def base_request(run_id: str, output_root: Path, summary_path: Path) -> dict:
...
def matrix_requests(review_root: Path) -> list[dict]:
    ...
    specs = [
        {
            "cell_id": "off_baseline",
            "run_id": "caustic_phase7_off_baseline",
            "inspection": {
                "caustic_mode": "off",
        ...
        {
            "cell_id": "surface_cache_default",
            "run_id": "caustic_phase7_surface_cache_default",
            "inspection": {
        ...
        {
            "cell_id": "surface_cache_calibrated",
            "run_id": "caustic_phase7_surface_cache_calibrated",
            "inspection": {
                "caustic_mode": "transport",
                "caustic_surface_enabled": True,
                "caustic_sidecar_enabled": False,
        ...
        {
            "cell_id": "combined_calibrated_analytic",
            "run_id": "caustic_phase7_combined_calibrated_analytic",
            "inspection": {
                "caustic_mode": "transport",
                "caustic_surface_enabled": True,
    ...
    for spec in specs:
        ...
        request = base_request(spec["run_id"], output_root, summary_path)
...
def metric_deltas(runs: dict[str, dict]) -> dict:
    baseline = runs["off_baseline"]["metrics"]
...
def frame_delta_metrics(runs: dict[str, dict]) -> dict:
    if "off_baseline" not in runs:
        return {}
    baseline_path = Path(runs["off_baseline"]["frame_path"])
...
def validate_cell(cell_id: str, digest: dict) -> list[str]:
    ...
    elif cell_id == "surface_cache_default":
        if not digest["transport_active"]:
            failures.append("default surface-cache cell did not activate transport")
        if digest["caustic_sidecar_enabled"]:
            failures.append("default surface-cache cell unexpectedly enabled analytic sidecar")
        if digest["surface_cache_record_count"] <= 0 or digest["surface_cache_deposit_accepted"] <= 0:
            failures.append("default surface-cache cell did not populate receiver records")
        if digest["surface_cache_sample_contributing_count"] <= 0:
            failures.append("default surface-cache cell did not contribute receiver samples")
        if digest["surface_caustic_sampled_radiance_sum"] <= 0.0:
            failures.append("default surface-cache cell sampled zero surface caustic radiance")
    elif cell_id == "surface_cache_calibrated":
        if not digest["transport_active"]:
            failures.append("calibrated surface-cache cell did not activate transport")
        if digest["caustic_sidecar_enabled"]:
            failures.append("calibrated surface-cache cell unexpectedly enabled analytic sidecar")
        if digest["surface_footprint_scale"] < 4.9 or digest["surface_radiance_scale"] < 2.4:
            failures.append("calibrated surface-cache cell did not report requested calibration scales")
        if digest["surface_cache_sample_contributing_count"] <= 0:
            failures.append("calibrated surface-cache cell did not contribute receiver samples")
    elif cell_id == "combined_calibrated_analytic":
        if not digest["transport_active"] or not digest["caustic_sidecar_enabled"]:
            failures.append("combined calibrated cell did not activate both transport and analytic sidecar")
        if digest["surface_cache_sample_contributing_count"] <= 0:
            failures.append("combined calibrated cell did not contribute surface-cache samples")
        if digest["caustic_sidecar_contributing_samples"] <= 0:
            failures.append("combined calibrated cell did not contribute sidecar samples")
    return failures
...
def write_index(path: Path, report: dict) -> None:
    ...
    lines = [
        "# Phase 7 Surface Caustic Calibration Matrix",
        "",
...
def main() -> int:
    ...
    deltas = metric_deltas(runs_by_cell) if "off_baseline" in runs_by_cell else {}
    frame_deltas = frame_delta_metrics(runs_by_cell) if "off_baseline" in runs_by_cell else {}
    default_frame = frame_deltas.get("surface_cache_default", {})
    calibrated_frame = frame_deltas.get("surface_cache_calibrated", {})
    if runs_by_cell.get("surface_cache_default") and default_frame.get("positive_pixel_count", 0) <= 0:
        failures.append("surface_cache_default: rendered frame did not change versus off baseline")
    if runs_by_cell.get("surface_cache_calibrated"):
        if calibrated_frame.get("positive_pixel_count", 0) <= 0:
            failures.append("surface_cache_calibrated: rendered frame did not change versus off baseline")
        if calibrated_frame.get("positive_pixel_count", 0) <= max(0, default_frame.get("positive_pixel_count", 0)) * 2:
            failures.append("surface_cache_calibrated: calibrated footprint did not broaden positive frame delta")
        if calibrated_frame.get("mean_luma_delta", 0.0) <= default_frame.get("mean_luma_delta", 0.0):
            failures.append("surface_cache_calibrated: calibrated mean frame delta did not exceed default")
    report = {
        "schema_version": "ray_tracing_spatial_caustic_phase7_calibration_matrix_v1",
    ...
    write_json(review_root / "phase7_surface_caustic_calibration_matrix_report.json", report)
    write_index(review_root / "phase7_surface_caustic_calibration_matrix_index.md", report)
    print(review_root / "phase7_surface_caustic_calibration_matrix_report.json")
