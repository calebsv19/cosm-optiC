#!/usr/bin/env python3
"""Run the Phase 6 surface-caustic receiver metrics A/B matrix."""
...
def default_review_root() -> Path:
    return workspace_root() / "_private_workspace_artifacts" / "agent_runs" / "ray_tracing" / "caustic_phase6_surface_matrix"
    ...
    specs = [
        {
            ...
            "run_id": "caustic_phase6_off_baseline",
            "inspection": {
                "caustic_mode": "off",
        ...
        {
            "cell_id": "analytic_only",
            "run_id": "caustic_phase6_analytic_only",
            "inspection": {
                "caustic_mode": "analytic",
                "caustic_surface_enabled": False,
        ...
        {
            "cell_id": "surface_cache_only",
            "run_id": "caustic_phase6_transport_surface_cache_only",
            "inspection": {
        ...
        {
            "cell_id": "combined_surface_analytic",
            "run_id": "caustic_phase6_combined_surface_cache_analytic",
            "inspection": {
                "caustic_mode": "transport",
                "caustic_surface_enabled": True,
                "caustic_sidecar_enabled": True,
...
def validate_cell(cell_id: str, digest: dict) -> list[str]:
    ...
    elif cell_id == "analytic_only":
        if not digest["caustic_sidecar_enabled"]:
            failures.append("analytic-only cell did not enable the sidecar")
        if digest["caustic_sidecar_contributing_samples"] <= 0:
            failures.append("analytic-only cell had no contributing sidecar samples")
        if digest["transport_active"] or digest["surface_cache_record_count"] != 0:
            failures.append("analytic-only cell unexpectedly activated transport surface cache")
    elif cell_id == "surface_cache_only":
    ...
    elif cell_id == "combined_surface_analytic":
        if not digest["transport_active"] or not digest["caustic_sidecar_enabled"]:
            failures.append("combined cell did not activate both transport and analytic sidecar")
        if digest["surface_cache_sample_contributing_count"] <= 0:
            failures.append("combined cell did not contribute surface-cache samples")
        if digest["caustic_sidecar_contributing_samples"] <= 0:
            failures.append("combined cell did not contribute sidecar samples")
    return failures
...
def write_index(path: Path, report: dict) -> None:
    ...
    lines = [
        "# Phase 6 Surface Caustic Receiver Matrix",
        "",
...
def main() -> int:
    ...
    report = {
        "schema_version": "ray_tracing_spatial_caustic_phase6_surface_matrix_v1",
    ...
    write_json(review_root / "phase6_surface_caustic_matrix_report.json", report)
    write_index(review_root / "phase6_surface_caustic_matrix_index.md", report)
    print(review_root / "phase6_surface_caustic_matrix_report.json")
