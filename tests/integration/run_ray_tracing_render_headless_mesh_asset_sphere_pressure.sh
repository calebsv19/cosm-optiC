...
REQUEST="$ROOT_DIR/tests/fixtures/agent_render_mesh_asset_sphere_pressure_request.json"
OUT_ROOT="$ROOT_DIR/build/agent_runs/ray_tracing/mesh_asset_sphere_pressure_mrt6"
...
assert bvh["triangle_count"] == 3974, bvh["triangle_count"]
...
entry = audit["obj_sphere_pressure"]
assert entry["triangle_count"] == 3968, entry["triangle_count"]
