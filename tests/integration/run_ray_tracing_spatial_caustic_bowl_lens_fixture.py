#!/usr/bin/env python3
"""Run a physical mist visual for the analytic bowl-lens transport path."""
...
def write_bowl_runtime_mesh_asset(mesh_dir: Path) -> Path:
    ...
    for i in range(segments):
        n = (i + 1) % segments
        triangles.append({"a": 0, "b": front_start + n, "c": front_start + i, "surface_group_id": "bowl_face"})
        triangles.append({"a": 1, "b": back_start + i, "c": back_start + n, "surface_group_id": "bowl_face"})
        triangles.append({"a": front_start + i, "b": front_start + n, "c": back_start + n, "surface_group_id": "bowl_rim"})
        triangles.append({"a": front_start + i, "b": back_start + n, "c": back_start + i, "surface_group_id": "bowl_rim"})
    asset = {
        "schema_family": "codework_geometry",
        "schema_variant": "mesh_asset_runtime_v1",
        "schema_version": 1,
        "asset_id": "asset_caustic_bowl_lens",
        "source_asset_id": "generated_analytic_bowl_fixture",
    ...
    mesh_path = mesh_dir / "asset_caustic_bowl_lens.runtime.json"
    write_json(mesh_path, asset)
    return mesh_path


def write_bowl_scene(review_root: Path) -> Path:
    scene_path = sphere_mist.write_visual_scene(review_root)
    scene = load_json(scene_path)
    scene["scene_id"] = "caustic_bowl_lens_fixture_v1"
    mesh_dir = scene_path.parent / "assets" / "mesh_assets"
    write_bowl_runtime_mesh_asset(mesh_dir)
    for obj in scene.get("objects", []):
        if obj.get("object_id") == "high_quality_glass_sphere":
            obj["object_id"] = "authored_water_bowl_lens"
            transform = obj.setdefault("transform", {})
            transform["position"] = {"x": 0.0, "y": 0.0, "z": 1.38}
            transform["scale"] = {"x": 0.76, "y": 0.34, "z": 0.70}
            obj["geometry_ref"] = {
                "kind": "mesh_asset",
                "id": "asset_caustic_bowl_lens",
                "variant": "runtime_default",
            }
            obj["extensions"] = {
                "line_drawing": {
                    "runtime_mesh_path": "assets/mesh_assets/asset_caustic_bowl_lens.runtime.json",
                }
            }
    ...
    for material in authoring.get("object_materials", []):
        if material.get("object_id") == "high_quality_glass_sphere":
            material["object_id"] = "authored_water_bowl_lens"
...
def request_for_cell(review_root: Path,
    ...
    elif cell_id == "volume_analytic_bowl_lens":
        request["inspection"].update({
            "caustic_mode": "transport",
            "caustic_volume_enabled": True,
            "caustic_surface_enabled": False,
            ...
            "caustic_max_path_depth": 2,
            "caustic_surface_receiver_fallback_enabled": False,
            "caustic_transport_emission_policy": "analytic_bowl_lens",
        })
    else:
        raise ValueError(f"unknown bowl fixture cell: {cell_id}")
...
def bowl_digest(summary: dict) -> dict:
    digest = sphere_mist.caustic_digest(summary)
    state = summary.get("inspection", {}).get("caustic_state", {})
    digest.update({
        "transport_analytic_bowl_lens_resolved_count": int(
            state.get("transport_analytic_bowl_lens_resolved_count", 0)
        ),
        "transport_analytic_bowl_lens_rejected_count": int(
            state.get("transport_analytic_bowl_lens_rejected_count", 0)
        ),
        "transport_analytic_bowl_lens_evaluated_paths": int(
            state.get("transport_analytic_bowl_lens_evaluated_path_count", 0)
        ),
        "transport_analytic_bowl_lens_emitted_paths": int(
            state.get("transport_analytic_bowl_lens_emitted_path_count", 0)
        ),
        "transport_analytic_bowl_lens_sample_weight": float(
            state.get("transport_analytic_bowl_lens_sample_weight", 0.0)
        ),
        "transport_analytic_bowl_lens_total_sample_weight": float(
            state.get("transport_analytic_bowl_lens_total_sample_weight", 0.0)
        ),
    })
    return digest
...
def validate_cell(cell_id: str, digest: dict) -> list[str]:
    ...
    if cell_id == "volume_analytic_bowl_lens":
        if digest.get("transport_analytic_bowl_lens_resolved_count", 0) <= 0:
            failures.append("analytic bowl-lens cell did not resolve a bowl descriptor")
        if digest.get("transport_analytic_bowl_lens_rejected_count", 0) != 0:
            failures.append("analytic bowl-lens cell reported descriptor rejection")
        if digest.get("transport_analytic_bowl_lens_evaluated_paths", 0) <= 0:
            failures.append("analytic bowl-lens cell did not evaluate analytic paths")
        if digest.get("transport_analytic_bowl_lens_emitted_paths", 0) <= 0:
            failures.append("analytic bowl-lens cell did not emit analytic paths")
        if digest.get("transport_analytic_sphere_lens_resolved_count", 0) != 0:
            failures.append("analytic bowl-lens cell unexpectedly resolved the sphere policy")
        if digest.get("transport_analytic_cylinder_lens_resolved_count", 0) != 0:
            failures.append("analytic bowl-lens cell unexpectedly resolved the cylinder policy")
        if digest.get("transport_analytic_prism_lens_resolved_count", 0) != 0:
            failures.append("analytic bowl-lens cell unexpectedly resolved the prism policy")
    return failures
...
def bowl_curved_interface_review(report: dict) -> dict:
    cell_id = "volume_analytic_bowl_lens"
    ...
    entry_normal_span = focus.get("entry_normal_bounds", {}).get("span", {})
    exit_normal_span = focus.get("exit_normal_bounds", {}).get("span", {})
    shape_counts = focus.get("lens_shape_kind_counts", {})
    positive_pixels = float(delta.get("positive_pixel_count", 0.0))
    entry_lateral_span = math.sqrt(
        float(entry_normal_span.get("x", 0.0)) * float(entry_normal_span.get("x", 0.0))
        + float(entry_normal_span.get("y", 0.0)) * float(entry_normal_span.get("y", 0.0))
    )
    exit_lateral_span = math.sqrt(
        float(exit_normal_span.get("x", 0.0)) * float(exit_normal_span.get("x", 0.0))
        + float(exit_normal_span.get("y", 0.0)) * float(exit_normal_span.get("y", 0.0))
    )

    review = {
        "cell_id": cell_id,
        "path_count": int(focus.get("path_count", 0)),
        "bowl_shape_count": int(shape_counts.get("bowl", 0)),
        "entry_normal_span": entry_normal_span,
        "exit_normal_span": exit_normal_span,
        "entry_normal_lateral_span": entry_lateral_span,
        "exit_normal_lateral_span": exit_lateral_span,
        "positive_pixel_count": positive_pixels,
        "curved_interface_positive": False,
    }
    review["curved_interface_positive"] = (
        review["path_count"] > 0
        and review["bowl_shape_count"] > 0
        and review["positive_pixel_count"] > 0
        and review["entry_normal_lateral_span"] >= 0.05
        and review["exit_normal_lateral_span"] <= 0.001
    )
    return review
...
def write_index(path: Path, report: dict) -> None:
    lines = [
        "# Spatial Caustic Bowl Lens Fixture",
        "",
    ...
    lines.extend([
        "",
        "## Bowl Curved Interface Review",
        "",
    ])
    review = report.get("bowl_curved_interface_review", {})
    if review:
        lines.append(
            f"- curved interface positive: `{review.get('curved_interface_positive', False)}`; "
            f"paths `{review.get('path_count', 0)}`; "
            f"bowl shape count `{review.get('bowl_shape_count', 0)}`; "
            f"entry normal lateral span `{review.get('entry_normal_lateral_span', 0.0):.4f}`; "
            f"exit normal lateral span `{review.get('exit_normal_lateral_span', 0.0):.4f}`; "
    ...
    for run in report["runs"]:
        ...
        lines.append(
            ...
            f"bowl resolved `{digest.get('transport_analytic_bowl_lens_resolved_count', 0)}`, "
            f"bowl emitted `{digest.get('transport_analytic_bowl_lens_emitted_paths', 0)}`, "
...
def main() -> int:
    ...
    scene_path = write_bowl_scene(review_root)
    ...
    contact_sheet_path = review_root / "bowl_lens_fixture_contact_sheet.png"
    sphere_mist.write_contact_sheet(contact_sheet_path, runs)
    delta_artifacts = write_delta_artifacts(review_root, runs_by_cell)
    report = {
        "schema_version": "ray_tracing_spatial_caustic_bowl_lens_fixture_v1",
    ...
    report["bowl_curved_interface_review"] = bowl_curved_interface_review(report)
    if not report["bowl_curved_interface_review"].get("curved_interface_positive", False):
        report["failures"].append("volume_analytic_bowl_lens: curved interface review did not pass")
        report["passed"] = False
    report_path = review_root / "bowl_lens_fixture_report.json"
    write_json(report_path, report)
    write_index(review_root / "bowl_lens_fixture_index.md", report)
    print(report_path)
