#!/usr/bin/env python3
"""Run a physical mist visual for the analytic prism-lens transport path."""
...
def write_prism_runtime_mesh_asset(mesh_dir: Path) -> Path:
    ...
    for x in (-1.0, 1.0):
        for point in cross_section:
            vertices.append({"x": x, "y": point["y"], "z": point["z"]})
    triangles = [
        {"a": 0, "b": 3, "c": 4, "surface_group_id": "prism_side"},
        {"a": 0, "b": 4, "c": 1, "surface_group_id": "prism_side"},
        {"a": 1, "b": 4, "c": 5, "surface_group_id": "prism_side"},
        {"a": 1, "b": 5, "c": 2, "surface_group_id": "prism_side"},
        {"a": 2, "b": 5, "c": 3, "surface_group_id": "prism_side"},
        {"a": 2, "b": 3, "c": 0, "surface_group_id": "prism_side"},
        {"a": 0, "b": 1, "c": 2, "surface_group_id": "prism_cap"},
        {"a": 3, "b": 5, "c": 4, "surface_group_id": "prism_cap"},
    ]
    asset = {
        "schema_family": "codework_geometry",
        "schema_variant": "mesh_asset_runtime_v1",
        "schema_version": 1,
        "asset_id": "asset_caustic_triangular_prism",
        "source_asset_id": "generated_analytic_prism_fixture",
        ...
        "local_bounds": {
            "min": {"x": -1.0, "y": -0.95, "z": -0.58},
            "max": {"x": 1.0, "y": 0.95, "z": 0.84},
        },
        "mesh": {
    ...
    mesh_path = mesh_dir / "asset_caustic_triangular_prism.runtime.json"
    write_json(mesh_path, asset)
    return mesh_path


def write_prism_scene(review_root: Path) -> Path:
    scene_path = sphere_mist.write_visual_scene(review_root)
    scene = load_json(scene_path)
    scene["scene_id"] = "caustic_prism_lens_fixture_v1"
    mesh_dir = scene_path.parent / "assets" / "mesh_assets"
    write_prism_runtime_mesh_asset(mesh_dir)
    for obj in scene.get("objects", []):
        if obj.get("object_id") == "high_quality_glass_sphere":
            obj["object_id"] = "authored_glass_prism_lens"
            transform = obj.setdefault("transform", {})
            transform["position"] = {"x": 0.0, "y": 0.0, "z": 1.38}
            transform["scale"] = {"x": 1.05, "y": 0.46, "z": 0.46}
            obj["geometry_ref"] = {
                "kind": "mesh_asset",
                "id": "asset_caustic_triangular_prism",
                "variant": "runtime_default",
            }
            obj["extensions"] = {
                "line_drawing": {
                    "runtime_mesh_path": "assets/mesh_assets/asset_caustic_triangular_prism.runtime.json",
                }
            }
    ...
    for material in authoring.get("object_materials", []):
        if material.get("object_id") == "high_quality_glass_sphere":
            material["object_id"] = "authored_glass_prism_lens"
            material["object_color"] = sphere_mist.rgb_u24(238, 244, 255)
            material["alpha"] = 0.36
...
def request_for_cell(review_root: Path,
    ...
    if cell_id == "mist_no_caustic":
    ...
    elif cell_id == "volume_analytic_prism_lens":
        request["inspection"].update({
            "caustic_mode": "transport",
            "caustic_volume_enabled": True,
            "caustic_surface_enabled": False,
        ...
        raise ValueError(f"unknown prism fixture cell: {cell_id}")
...
def prism_digest(summary: dict) -> dict:
    digest = sphere_mist.caustic_digest(summary)
    state = summary.get("inspection", {}).get("caustic_state", {})
    digest.update({
        "transport_analytic_prism_lens_resolved_count": int(
            state.get("transport_analytic_prism_lens_resolved_count", 0)
        ),
        "transport_analytic_prism_lens_rejected_count": int(
            state.get("transport_analytic_prism_lens_rejected_count", 0)
        ),
        "transport_analytic_prism_lens_evaluated_paths": int(
            state.get("transport_analytic_prism_lens_evaluated_path_count", 0)
        ),
        "transport_analytic_prism_lens_emitted_paths": int(
            state.get("transport_analytic_prism_lens_emitted_path_count", 0)
        ),
        "transport_analytic_prism_lens_sample_weight": float(
            state.get("transport_analytic_prism_lens_sample_weight", 0.0)
        ),
        "transport_analytic_prism_lens_total_sample_weight": float(
            state.get("transport_analytic_prism_lens_total_sample_weight", 0.0)
        ),
    })
    return digest
...
def validate_cell(cell_id: str, digest: dict) -> list[str]:
    ...
    if cell_id == "volume_analytic_prism_lens":
        if digest.get("transport_analytic_prism_lens_resolved_count", 0) <= 0:
            failures.append("analytic prism-lens cell did not resolve a prism descriptor")
        if digest.get("transport_analytic_prism_lens_rejected_count", 0) != 0:
            failures.append("analytic prism-lens cell reported descriptor rejection")
        if digest.get("transport_analytic_prism_lens_evaluated_paths", 0) <= 0:
            failures.append("analytic prism-lens cell did not evaluate analytic paths")
        if digest.get("transport_analytic_prism_lens_emitted_paths", 0) <= 0:
            failures.append("analytic prism-lens cell did not emit analytic paths")
        if digest.get("transport_analytic_sphere_lens_resolved_count", 0) != 0:
            failures.append("analytic prism-lens cell unexpectedly resolved the sphere policy")
        if digest.get("transport_analytic_cylinder_lens_resolved_count", 0) != 0:
            failures.append("analytic prism-lens cell unexpectedly resolved the cylinder policy")
    return failures
...
def prism_deflection_review(report: dict) -> dict:
    cell_id = "volume_analytic_prism_lens"
    ...
    direction = focus.get("exit_direction_spread_degrees", {}).get("mean_direction", {})
    ...
    mean_x = float(direction.get("x", 0.0))
    mean_y = float(direction.get("y", 0.0))
    mean_lateral = math.sqrt(mean_x * mean_x + mean_y * mean_y)

    review = {
        "cell_id": cell_id,
        "path_count": int(focus.get("path_count", 0)),
        "prism_shape_count": int(shape_counts.get("prism", 0)),
        "mean_exit_direction": direction,
        "mean_exit_lateral": mean_lateral,
        "mean_exit_abs_x": abs(mean_x),
        "receiver_span_x": float(receiver_span.get("x", 0.0)),
        "receiver_span_y": float(receiver_span.get("y", 0.0)),
        "positive_pixel_count": positive_pixels,
        "prism_deflection_positive": False,
    }
    review["prism_deflection_positive"] = (
        review["path_count"] > 0
        and review["prism_shape_count"] > 0
        and review["positive_pixel_count"] > 0
        and (review["mean_exit_abs_x"] >= 0.02 or review["receiver_span_x"] >= 0.05)
    )
    return review
...
def write_index(path: Path, report: dict) -> None:
    lines = [
        "# Spatial Caustic Prism Lens Fixture",
        "",
    ...
    lines.extend([
        "",
        "## Prism Deflection Review",
        "",
    ])
    review = report.get("prism_deflection_review", {})
    if review:
        lines.append(
            f"- prism deflection positive: `{review.get('prism_deflection_positive', False)}`; "
            f"paths `{review.get('path_count', 0)}`; "
            f"prism shape count `{review.get('prism_shape_count', 0)}`; "
            f"mean exit abs x `{review.get('mean_exit_abs_x', 0.0):.4f}`; "
            f"receiver span x/y `{review.get('receiver_span_x', 0.0):.4f}`/"
            f"`{review.get('receiver_span_y', 0.0):.4f}`; "
    ...
    for run in report["runs"]:
        ...
        lines.append(
            ...
            f"prism resolved `{digest.get('transport_analytic_prism_lens_resolved_count', 0)}`, "
            f"prism emitted `{digest.get('transport_analytic_prism_lens_emitted_paths', 0)}`, "
...
def main() -> int:
    ...
    scene_path = write_prism_scene(review_root)
    ...
    contact_sheet_path = review_root / "prism_lens_fixture_contact_sheet.png"
    sphere_mist.write_contact_sheet(contact_sheet_path, runs)
    delta_artifacts = write_delta_artifacts(review_root, runs_by_cell)
    report = {
        "schema_version": "ray_tracing_spatial_caustic_prism_lens_fixture_v1",
    ...
    report["prism_deflection_review"] = prism_deflection_review(report)
    if not report["prism_deflection_review"].get("prism_deflection_positive", False):
        report["failures"].append("volume_analytic_prism_lens: prism deflection review did not pass")
        report["passed"] = False
    report_path = review_root / "prism_lens_fixture_report.json"
    write_json(report_path, report)
    write_index(review_root / "prism_lens_fixture_index.md", report)
    print(report_path)
