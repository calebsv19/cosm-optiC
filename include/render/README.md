...
- `runtime_native_3d_render.h` – Native live-render entrypoint for the bounded runtime lane. It builds `RuntimeScene3D` at the current playback `t`, applies live light/camera overrides, and shades the frame into the luminance buffer through the native direct-light path.
