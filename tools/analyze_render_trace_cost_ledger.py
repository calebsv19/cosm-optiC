...
def transmission_summary(ledger: dict[str, Any], top: int) -> dict[str, Any]:
    policy = ledger.get("transmission_path_policy")
    if not isinstance(policy, dict):
        return {}
    sample_evaluations = as_int(policy.get("sample_evaluations"))
    ray_traces = as_int(policy.get("ray_traces"))
    path_evaluations = as_int(policy.get("path_evaluations"))
    return {
        "path_evaluations": path_evaluations,
        "requested_samples": as_int(policy.get("requested_samples")),
        "sample_evaluations": sample_evaluations,
        "contributing_samples": as_int(policy.get("contributing_samples")),
        "receiver_samples": as_int(policy.get("receiver_samples")),
        "ray_traces": ray_traces,
        "hit_surfaces": as_int(policy.get("hit_surfaces")),
        "transparent_surface_hits": as_int(policy.get("transparent_surface_hits")),
        "receiver_hits": as_int(policy.get("receiver_hits")),
        "ray_traces_per_sample": (
            float(ray_traces) / float(sample_evaluations)
            if sample_evaluations > 0 else 0.0
        ),
        "samples_per_path": (
            float(sample_evaluations) / float(path_evaluations)
            if path_evaluations > 0 else 0.0
        ),
        "avg_ray_traces_per_sample": float(policy.get("avg_ray_traces_per_sample", 0.0) or 0.0),
        "max_ray_traces_in_sample": as_int(policy.get("max_ray_traces_in_sample")),
        "avg_transparent_surfaces_per_sample": float(
            policy.get("avg_transparent_surfaces_per_sample", 0.0) or 0.0
        ),
        "max_transparent_surfaces_in_sample": as_int(
            policy.get("max_transparent_surfaces_in_sample")
        ),
        "sources": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("source_counts"))[:top]
        ],
        "source_samples": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("source_sample_counts"))[:top]
        ],
        "terminations": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("termination_counts"))[:top]
        ],
        "surface_kinds": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("surface_kind_counts"))[:top]
        ],
        "transparent_surface_materials": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("transparent_surface_material_counts"))[:top]
        ],
        "receiver_materials": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("receiver_material_counts"))[:top]
        ],
        "terminal_depths": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("terminal_depth_counts"))[:top]
        ],
        "ray_depths": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("ray_depth_counts"))[:top]
        ],
        "throughput_buckets": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("throughput_bucket_counts"))[:top]
        ],
        "contribution_buckets": [
            {"name": name, "count": count}
            for name, count in sorted_counts(policy.get("contribution_bucket_counts"))[:top]
        ],
    }
...
def summarize(path: Path, top: int) -> dict[str, Any]:
    ...
    return {
        ...
        "transmission_path_policy": transmission_summary(ledger, top),
...
def print_text_report(reports: list[dict[str, Any]]) -> None:
    for index, report in enumerate(reports):
        ...
        transmission = report.get("transmission_path_policy")
        if isinstance(transmission, dict) and transmission:
            print("  Transmission path policy:")
            print(f"    path_evaluations: {format_count(as_int(transmission['path_evaluations']))}")
            print(f"    requested_samples: {format_count(as_int(transmission['requested_samples']))}")
            print(f"    sample_evaluations: {format_count(as_int(transmission['sample_evaluations']))}")
            print(f"    contributing_samples: {format_count(as_int(transmission['contributing_samples']))}")
            print(f"    receiver_samples: {format_count(as_int(transmission['receiver_samples']))}")
            print(f"    ray_traces: {format_count(as_int(transmission['ray_traces']))}")
            print(f"    ray_traces_per_sample: {float(transmission['ray_traces_per_sample']):.3f}")
            print(
                "    avg_transparent_surfaces_per_sample: "
                f"{float(transmission['avg_transparent_surfaces_per_sample']):.3f}"
            )
            print(
                "    max_transparent_surfaces_in_sample: "
                f"{format_count(as_int(transmission['max_transparent_surfaces_in_sample']))}"
            )
            print_section("    Sources", transmission["sources"], total_label="path evaluations")
            print_section("    Source samples", transmission["source_samples"], total_label="samples")
            print_section("    Terminations", transmission["terminations"], total_label="samples")
            print_section("    Surface kinds", transmission["surface_kinds"], total_label="surface hits")
            print_section(
                "    Transparent surface materials",
                transmission["transparent_surface_materials"],
                total_label="transparent surface hits",
            )
            print_section(
                "    Receiver materials",
                transmission["receiver_materials"],
                total_label="receiver hits",
            )
            print_section("    Terminal depths", transmission["terminal_depths"], total_label="samples")
            print_section("    Ray depths", transmission["ray_depths"], total_label="transmission rays")
            print_section(
                "    Throughput buckets",
                transmission["throughput_buckets"],
                total_label="samples",
            )
            print_section(
                "    Contribution buckets",
                transmission["contribution_buckets"],
                total_label="samples",
            )
